...
function MovieCard({item}) {
  ...
  return (
    <div className='space-x-4 mx-2 w-full h-96 gap-20 space-y-4 justify-center flex transition-all duration-300 ' key =  {item.id} onClick={handleClick}>
    ...
    {item.poster_path === null || undefined ? <img src = {require("../assets/nature.jpg")} alt = "poster movie" width="500" height="700" className='h-72 rounded-md w-100 animate__animated animate__fadeInBottomRight animate__slow'/>: <img src = {posterPath} alt = "poster movie" className='w-100 h-100 rounded-md animate__animated animate__fadeInBottomRight animate__slow'/>} 
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#68fc23" class="bi bi-youtube " viewBox="0 0 16 16">
        ...
        <h1 className = "text-center py-4 text-black font-bold dark:text-white" >{item.original_title === undefined ? item.name:item.original_title}</h1>
     </div>
    </div>
  )
}
